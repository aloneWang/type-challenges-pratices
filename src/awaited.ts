...
type MyAwaited<T> = T extends object & { then: (onfulfilled: infer F, ...args: infer _) => any} ?
  // 具有 thenable 属性
  F extends (f: infer F, ...args: infer _) => any ? 
    // 递归判断 函数参数 f
    MyAwaited<F> 
  : never  // 回调函数不匹配
: T
...
/*
type _MyAwaited<T extends PromiseLike<any>> = T extends PromiseLike<infer A>
